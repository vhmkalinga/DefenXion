...
import { FileText, Download, Calendar, Filter, Terminal, Trash2, Eye, Search, ChevronLeft, ChevronRight, BarChart3, Shield, AlertTriangle, X, Loader2, Printer } from 'lucide-react';
...
export function ReportsLogs() {
  ...
  const handleDownloadPDF = async (r: any) => {
    try {
      const fullReport = await getReportDetail(r.report_id);
      
      const attacksHtml = fullReport.top_attack_sources?.length 
        ? fullReport.top_attack_sources.map((s: any) => `<tr><td>${s.ip}</td><td>${s.count}</td></tr>`).join('') 
        : '<tr><td colspan="2">No attack sources recorded.</td></tr>';
        
      const actionsHtml = fullReport.action_distribution?.length 
        ? fullReport.action_distribution.map((a: any) => `<tr><td>${a.action}</td><td>${a.count}</td></tr>`).join('') 
        : '<tr><td colspan="2">No actions recorded.</td></tr>';

      const element = document.createElement('div');
      element.innerHTML = `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #333; padding: 40px; line-height: 1.6; background: #fff;">
            <h1 style="color: #1a1a1a; border-bottom: 2px solid #eee; padding-bottom: 10px; margin-top: 0;">${fullReport.title}</h1>
            <div style="margin-bottom: 30px; color: #555; font-size: 14px; background: #f9f9f9; padding: 15px; border-radius: 6px;">
                <strong>Report ID:</strong> ${fullReport.report_id}<br/>
                <strong>Generated At:</strong> ${fullReport.generated_at}<br/>
                <strong>Generated By:</strong> ${fullReport.generated_by}<br/>
                <strong>Period:</strong> ${fullReport.period}
            </div>

            <div style="display: flex; gap: 15px; margin-bottom: 30px;">
                <div style="flex: 1; border: 1px solid #eaeaea; padding: 15px; border-radius: 8px; background: #fafafa;">
                    <div style="font-size: 12px; color: #666; text-transform: uppercase; letter-spacing: 0.5px;">Total Detections</div>
                    <div style="font-size: 28px; font-weight: 700; color: #1a1a1a; margin-top: 5px;">${fullReport.summary?.total_detections || 0}</div>
                </div>
                <div style="flex: 1; border: 1px solid #eaeaea; padding: 15px; border-radius: 8px; background: #fafafa;">
                    <div style="font-size: 12px; color: #666; text-transform: uppercase; letter-spacing: 0.5px;">IP Blocks</div>
                    <div style="font-size: 28px; font-weight: 700; color: #d73a49; margin-top: 5px;">${fullReport.summary?.total_blocks || 0}</div>
                </div>
            </div>
            
            <div style="display: flex; gap: 15px; margin-bottom: 30px;">
                <div style="flex: 1; border: 1px solid #eaeaea; padding: 15px; border-radius: 8px; background: #fafafa;">
                    <div style="font-size: 12px; color: #666; text-transform: uppercase; letter-spacing: 0.5px;">Total Alerts</div>
                    <div style="font-size: 28px; font-weight: 700; color: #1a1a1a; margin-top: 5px;">${fullReport.summary?.total_alerts || 0}</div>
                </div>
                <div style="flex: 1; border: 1px solid #eaeaea; padding: 15px; border-radius: 8px; background: #fafafa;">
                    <div style="font-size: 12px; color: #666; text-transform: uppercase; letter-spacing: 0.5px;">Active Alerts</div>
                    <div style="font-size: 28px; font-weight: 700; color: #1a1a1a; margin-top: 5px;">${fullReport.summary?.active_alerts || 0}</div>
                </div>
            </div>

            <div style="margin-top: 40px;">
                <h2 style="color: #444; font-size: 18px; margin-bottom: 15px; border-bottom: 1px solid #eee; padding-bottom: 5px;">Top Attack Sources</h2>
                <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
                    <tr>
                        <th style="border: 1px solid #ddd; padding: 10px 14px; text-align: left; font-size: 14px; background-color: #f5f5f5; color: #333; font-weight: 600;">IP Address</th>
                        <th style="border: 1px solid #ddd; padding: 10px 14px; text-align: left; font-size: 14px; background-color: #f5f5f5; color: #333; font-weight: 600;">Attack Count</th>
                    </tr>
                    ${attacksHtml.replace(/<td>/g, '<td style="border: 1px solid #ddd; padding: 10px 14px; text-align: left; font-size: 14px;">')}
                </table>
            </div>

            <div style="margin-top: 40px;">
                <h2 style="color: #444; font-size: 18px; margin-bottom: 15px; border-bottom: 1px solid #eee; padding-bottom: 5px;">Action Distribution</h2>
                <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
                    <tr>
                        <th style="border: 1px solid #ddd; padding: 10px 14px; text-align: left; font-size: 14px; background-color: #f5f5f5; color: #333; font-weight: 600;">Action Taken</th>
                        <th style="border: 1px solid #ddd; padding: 10px 14px; text-align: left; font-size: 14px; background-color: #f5f5f5; color: #333; font-weight: 600;">Count</th>
                    </tr>
                    ${actionsHtml.replace(/<td>/g, '<td style="border: 1px solid #ddd; padding: 10px 14px; text-align: left; font-size: 14px;">')}
                </table>
            </div>
            
            <div style="margin-top: 40px; font-size: 11px; color: #999; text-align: center; border-top: 1px solid #eee; padding-top: 10px;">
                Generated by DefenXion Security Platform
            </div>
        </div>
      `;
      ...
      const opt = {
        margin:       0.5,
        filename:     `${fullReport.report_id}.pdf`,
                  ...
                  <div key={r.report_id} style={{ ...CARD, padding:'18px 22px', transition:'border-color 0.2s' }}
                    ...
                    <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between' }}>
                      ...
                      <div style={{ display:'flex', gap:6, flexShrink:0, marginLeft:12 }}>
                        ...
                        <button onClick={() => handleDownloadPDF(r)} style={BTN('#BC8CFF')}><Printer size={12} /> PDF</button>
