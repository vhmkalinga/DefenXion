...
export function ReportsLogs() {
  ...
  const handleDownloadPDF = async (r: any) => {
    try {
      toast.info('Fetching report data…');
      const fullReport = await getReportDetail(r.report_id);
      toast.info('Opening print dialog…');
      await generateReportPDF(fullReport);
            ...
            <div style={{ ...CARD, padding:'24px 28px', marginTop:16, position:'relative', overflow:'hidden' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
                ...
                <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                  <button onClick={() => handleDownloadPDF(selectedReport)} style={{ display:'flex', alignItems:'center', gap:5, padding:'7px 14px', borderRadius:8, background:'rgba(188,140,255,0.1)', border:'1px solid rgba(188,140,255,0.25)', color:'#BC8CFF', fontSize:12, fontWeight:600, fontFamily:'inherit', cursor:'pointer' }}><Printer size={13} /> PDF</button>
